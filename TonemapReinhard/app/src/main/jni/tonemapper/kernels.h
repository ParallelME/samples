...
const char tonemapSource[] =
    ...
    "        int width) {                                   \n"
    "    int base = get_global_id(0) * width;               \n"
    "    float sum = 0.0f;                                  \n"
    "    float max = 0.0f;                                  \n"
    "                                                       \n"
    ...
    "        int width, int height, float key) {            \n"
    "    float sum = 0.0f;                                  \n"
    "    float max = 0.0f;                                  \n"
    "                                                       \n"
    "    for(int i = 0; i < height; ++i) {                  \n"
    "        sum += gData[width * i].s3;                    \n"
    "                                                       \n"
    "        float val = gData[width * i + 1].s3;           \n"
    "        if(val > max)                                  \n"
    "            max = val;                                 \n"
    "    }                                                  \n"
    "                                                       \n"
    "    // Save the scale factor on gData[0].s3.           \n"
    "    float average = exp(sum / (float)(width            \n"
    "        * height));                                    \n"
    "    float scaleFactor = key                            \n"
    "        * (1.0f / average);                            \n"
    ...
    "        __global uchar4 *gOut, float power) {          \n"
    "    int gid = get_global_id(0);                        \n"
    "    float4 in = gIn[gid];                              \n"
    "    uchar4 out;                                        \n"
    "    in = clamp(in, 0.0f, 1.0f);                        \n"
    "    out.x = (uchar) (255.0f * pow(in.s0, power));      \n"
    "    out.y = (uchar) (255.0f * pow(in.s1, power));      \n"
    "    out.z = (uchar) (255.0f * pow(in.s2, power));      \n"
    "    out.w = 255;                                       \n"
    "    gOut[gid] = out;                                   \n"
    "}                                                      \n";
