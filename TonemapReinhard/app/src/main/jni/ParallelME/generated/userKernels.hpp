...
const char userKernels[] =
	...
	"}\n"
	"\n"
	" float4 reduce2_func(float4 pixel1, float4 pixel2) {\n"
	"\n"
	"                if(fabs(pixel1.s3) < 0.000001f)\n"
	"                    pixel1.s3 = (float) log(0.00001f + pixel1.s0);\n"
	"                if(fabs(pixel2.s3) < 0.000001f)\n"
	"                    pixel2.s3 = (float) log(0.00001f + pixel2.s0);\n"
	...
	"            }\n"
	"\n"
	"__kernel void reduce2_tile(__global float4* PM_data, __global float4* PM_tile, int PM_width) {\n"
	"	int PM_gid = get_global_id(0);\n"
	"	int PM_base = PM_gid * PM_width;\n"
	"	float4 pixel1 = PM_data[PM_base];\n"
	"	float4 pixel2;\n"
	"	for (int i=1; i<PM_width; ++i) {\n"
	"		pixel2 = PM_data[PM_base + i];\n"
	"		pixel1 = reduce2_func(pixel1, pixel2);\n"
	"	}\n"
	"	PM_tile[PM_gid] = pixel1;\n"
	"}\n"
	"\n"
	"__kernel void reduce2(__global float4* PM_tile, __global float4* PM_ret, int PM_height) {\n"
	"	float4 pixel1 = PM_tile[0];\n"
	"	float4 pixel2;\n"
	"	for (int i=1; i<PM_height; ++i) {\n"
	"		pixel2 = PM_tile[i];\n"
	"		pixel1 = reduce2_func(pixel1, pixel2);\n"
	"	}\n"
	"	*PM_ret = pixel1;\n"
