...
const char userKernels[] =
	"__kernel void toFloat(__global uchar4 *PM_dataIn, __global float4 *PM_dataOut) {\n"
	...
	"}\n"
	"\n"
	"__kernel void iterator1(__global float4 *PM_data) {\n"
	"	int PM_gid = get_global_id(0);\n"
	...
	"}\n"
	"\n"
	"__kernel void iterator2(__global float4 *PM_data, float sum, __global float* PM_sum, float max, __global float* PM_max, int PM_width, int PM_height)\n"
	" {\n"
	...
	"}\n"
	"\n"
	"__kernel void iterator3(__global float4 *PM_data, float fScaleFactor, float fLmax2) {\n"
	"	int PM_gid = get_global_id(0);\n"
	"	float4 pixel = PM_data[PM_gid]; \n"
	"                \n"
	...
	"}\n"
	"\n"
	"__kernel void iterator4(__global float4 *PM_data) {\n"
	"	int PM_gid = get_global_id(0);\n"
	"	float4 pixel = PM_data[PM_gid]; \n"
	"                float _x, _y, _z, g, b;\n"
	"\n"
	...
	"}\n"
	"\n"
	"__kernel void iterator5(__global float4 *PM_data, float power) {\n"
	"	int PM_gid = get_global_id(0);\n"
	"	float4 pixel = PM_data[PM_gid]; \n"
	"                \n"
