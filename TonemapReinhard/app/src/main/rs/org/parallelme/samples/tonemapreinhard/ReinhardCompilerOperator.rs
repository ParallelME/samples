...
rs_allocation PM_gInputReduce2;
...
static float4 reduce2_func(float4 pixel1, float4 pixel2) {
                ...
                if(fabs(pixel1.s3) < 0.000001f)
                    pixel1.s3 = (float) log(0.000001f + pixel1.s0);
                if(fabs(pixel2.s3) < 0.000001f)
                    pixel2.s3 = (float) log(0.000001f + pixel2.s0);
...
float4 __attribute__((kernel)) reduce2_tile(uint32_t x) {
	float4 pixel1 = rsGetElementAt_float4(PM_gInputReduce2, x, 0);
	float4 pixel2;
	for (int i=1; i<PM_gInputYSizeReduce2; ++i) {
		pixel2 = rsGetElementAt_float4(PM_gInputReduce2, x, i);
